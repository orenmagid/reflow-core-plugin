...
**What this means for the build, in order.** While views are created through the connector, views 2, 4, 5 and 6 carry exact dates, they go stale as the calendar advances, and **re-stamping all four is the daily run's job — mandatory, and the run must name all four rather than "the dated views"** (a run that re-stamped only some left `Done (recently)` ten days stale, undetected, because a stale view looks exactly like a fresh one). An item deferred to a date that has since arrived falls out of view 2 *and* view 5 simultaneously and becomes invisible in both. `update_view` re-stamps an exact date cleanly (verified), so the mechanism works; what made it load-bearing was that nothing else could.

**Once views can be written through the supplemental connector** (act:08d1df6c) rather than the first-party one, all four filters become relative and self-maintaining, the re-stamp step is deleted outright, and the daily run's liveness/staleness warning drops from load-bearing to ordinary. **The two window semantics are decided (Oren, 2026-07-27): "Due soon" becomes *due within a month*, and "Done (recently)" becomes *done in the past week*** — both more natural to say to a client than the arbitrary fourteen and seven days they replace.
